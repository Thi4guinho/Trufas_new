...
export const SalesCalendar: React.FC<SalesCalendarProps> = ({ sales }) => {
  ...
  const renderDetailsModal = () => {
    ...
    if (details && details.total > 0) {
      ...
      const bestPm = Object.entries(details.payments).sort((a, b) => (b[1] as number) - (a[1] as number))[0]?.[0];
      topPayment = pmMap[bestPm] || bestPm || 'Nenhuma';
      
      const bestHour = Object.entries(details.hours).sort((a, b) => (b[1] as number) - (a[1] as number))[0]?.[0];
