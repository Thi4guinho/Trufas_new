...
export const Settings: React.FC<SettingsProps> = ({ settings, profile }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await addDoc(collection(db, 'audit_logs'), {
        ...
        details: `Empresa: ${businessName} | Telefone: ${businessPhone} `,
        date: Timestamp.now(),
        ownerId: profile?.companyId || auth.currentUser!.uid
      });
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-8 font-sans">
      <div className="bg-white dark:bg-zinc-900 p-8 md:p-10 rounded-[2.5rem] border border-[#141414]/5 dark:border-zinc-50/10 shadow-xl space-y-6">
        <div>
          <p className="text-[10px] font-black uppercase tracking-widest text-[#141414]/40 dark:text-zinc-400">Configurações Básicas</p>
          <h2 className="text-2xl md:text-3xl font-black tracking-tighter italic">Informações da Empresa</h2>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pb-6 border-b border-[#141414]/5 dark:border-zinc-50/10">
          <div>
            <label className="block text-[10px] font-black uppercase tracking-widest text-[#141414]/40 dark:text-zinc-400 mb-2">Nome Comercial</label>
            <input 
              type="text"
              value={businessName}
              onChange={(e) => setBusinessName(e.target.value)}
              placeholder="Ex: Produtos da Maria"
              className="w-full p-4 bg-[#F5F5F4] dark:bg-zinc-800 rounded-2xl font-bold border-none focus:ring-2 focus:ring-[#141414]/10 dark:ring-zinc-50/10 text-sm text-[#141414] dark:text-zinc-100"
              required
            />
          </div>

          <div>
            <label className="block text-[10px] font-black uppercase tracking-widest text-[#141414]/40 dark:text-zinc-400 mb-2">Telefone Comercial / WhatsApp</label>
            <input 
              type="tel"
              value={businessPhone}
          ...
          <div>
            <label className="block text-[10px] font-black uppercase tracking-widest text-[#141414]/40 dark:text-zinc-400 mb-2">Modelo de Negócio</label>
            <select
              value={businessModel}
        ...
        <div>
          <p className="text-[10px] font-black uppercase tracking-widest text-[#141414]/40 dark:text-zinc-400 mb-1">Preço Progressivo</p>
          <p className="text-[#141414]/60 dark:text-zinc-300 mb-6 text-xs leading-relaxed font-medium">
        ...
        <div className="space-y-4">
          {rules.length === 0 && (
            <div className="p-6 text-center border-2 border-dashed border-[#141414]/10 dark:border-zinc-50/20 rounded-2xl text-xs font-bold text-[#141414]/40 dark:text-zinc-400">
              Nenhuma faixa progressiva configurada.
            </div>
          )}

          {rules.map((rule, i) => (
            <div key={i} className="flex gap-4 items-end p-4 bg-[#F5F5F4] dark:bg-zinc-800 rounded-2xl border border-[#141414]/5 dark:border-zinc-50/10">
              <div className="flex-1">
                <label className="block text-[9px] font-black uppercase tracking-widest text-[#141414]/40 dark:text-zinc-400 mb-1.5">Qtd. Mínima (Un.)</label>
                <input 
                  type="number"
                  min="2"
                  ...
                  className="w-full p-3 bg-white dark:bg-zinc-900 rounded-xl font-bold border-none text-xs"
                />
              </div>
              <div className="flex-1">
                <label className="block text-[9px] font-black uppercase tracking-widest text-[#141414]/40 dark:text-zinc-400 mb-1.5">Preço Unitário Especial (R$)</label>
                <input 
                  type="number"
                  step="0.01"
                  min="0.1"
        ...
        <div className="pt-6 border-t border-[#141414]/10 dark:border-zinc-50/20 mt-8 mb-4">
        ...
        <div className="flex gap-4 pt-4">
          ...
          <button 
            ...
            className="flex-1 bg-[#F5F5F4] dark:bg-zinc-800 hover:bg-[#141414]/5 dark:bg-zinc-50/5 text-[#141414] dark:text-zinc-100 py-4 rounded-2xl font-black text-xs uppercase tracking-widest flex items-center justify-center gap-1 transition-all"
          >
          ...
          <button 
            ...
            onClick={handleSave}
            disabled={isSaving}
            className="flex-1 bg-[#141414] dark:bg-zinc-100 text-white dark:text-zinc-900 py-4 rounded-2xl font-black text-xs uppercase tracking-widest shadow-lg hover:bg-[#141414]/90 transition-all"
          >
